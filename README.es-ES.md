...
lo permite, va a conectar sin problema una base de datos real, llamar a una
API real, o buscar un `.env` real — nada lo impide por defecto. Este paquete
...
Cada afirmación de abajo se puede verificar en
`ai/mock-creator/CONTRACT.md` o `bin/check.mjs` — sin cifras inventadas,
solo lo que realmente pasa cuando lo corrés.

### Para diseñadores que prototipan con IA

- **Entregás la historia completa, no solo el mejor día.** `states.json` +
  ...
  aplicables, con motivo. Lo que entregás ya responde "¿y si no hay nada?" y
...
- **Un prototipo que se comporta como el producto.** Los estados de carga
  simulan latencia real en vez de resolver al instante, así que lo que le
  mostrás a un usuario ya trae la fricción que tendría el producto real.
- **La IA se queda como herramienta de prototipado, no como pasivo oculto.**
  ...
  sin que `mocks:check` falle — un "hacé que funcione" no puede convertirse
  en silencio en infraestructura real que nunca pediste y ahora tenés que
  justificar.
- **Diseñás una vez, entregás limpio.** Cada pantalla lee a través de un
  único punto de acceso, `src/data/`. Cuando llega el momento de ir a
  producción, ingeniería reescribe esas funciones — tus layouts y
  componentes se entregan intactos.

### Para programadores de frontend que lo reciben

- **Heredás código que funciona, no una foto de uno que funciona.** El
  punto de acceso a datos implica que la única reescritura está dentro de
  `src/data/*` — cambiás el mock por una llamada real y todo lo que está
  arriba (componentes, layouts, routing) sigue funcionando tal cual.
- **Los estados que normalmente descubrís en QA ya están construidos.**
  ...
  dato real.
- **No hay nada falso para desenredar primero.** Los límites duros del
  ...
  que la integración arranca desde un punto de acceso limpio, no de un
  juego de adivinar qué es real.

### Para programadores de backend que leen el handoff

- **Un primer spec de API, antes de escribir un solo endpoint.**
  `src/mocks/schemas/` ya nombra cada campo y tipo contra el que está
  construido el frontend — lo leés una vez y ya sabés la forma hacia la que
  tenés que construir.
- **`states.json` es tu checklist de aceptación.** Lo que está declarado
  `true` (resultados vacíos, errores, paginación) es un comportamiento que
  se espera que tu API soporte; lo que está marcado como no aplicable viene
  con un motivo adjunto, así que nunca tenés que adivinar qué se dejó
  afuera ni por qué.
- **El contrato de error, ya acordado.** El frontend ya está programado
  contra una forma específica de fixture de error — sabés exactamente qué
  sobre (envelope) devolver antes del día de integración, en vez de
  negociarlo en vivo.
...
- **Un gate real de CI, no una sugerencia de linter.** `mocks:check` no
  ...
  que se cruza el alcance — lo conectás a CI y una violación bloquea el
  merge, no espera a que alguien la note en la revisión.
- **El límite es estructural, no una política que alguien se olvida.**
...
Localmente, desde una copia clonada de este repo:
...
- **`init`** — escribe el contrato, los adaptadores y el validador en el
  ...
  qué crearía, qué ya es idéntico, y un diff de lo que divergió — y no
  escribe nada hasta que lo corras de nuevo con `--write`. `--write` solo
  crea los archivos que faltan; nunca toca uno que ya existe, sea idéntico o
  haya divergido.
...
- **`check-install`** — lee los digests que `init` registró en
  ...
  cambios, con deriva (editado localmente desde la instalación), desactualizado
  (la plantilla del propio paquete avanzó desde la instalación), o faltante.
...
Leé `ai/mock-creator/CONTRACT.md` después de instalar — es el documento
operativo al que apunta cada archivo de agente. Versión corta:
...
Agregá esto al `package.json` de cada prototipo (ajustando la ruta relativa
a `ai/mock-creator/bin/check.mjs`), y corrélo desde el directorio propio de
ese prototipo antes de considerar un cambio como terminado:
...
guía del contrato — `check.mjs` todavía no la exige automáticamente.
